...
export const Sidebar: FC<SidebarProps> = () => {
  ...
  const mapItems = (items: SidebarItem[]) => {
    return items.map((item) => {
      ...
      return (
        <li key={item.title} style={{ backgroundColor: isActive ? '#f5f5f5' : '', color: '#2b313b' }}>
          {isActive && <div style={{ position: 'absolute', left: '0px', color: '#2375e1' }}>&#8226;</div>}
          {!isActive && !isNested && <div style={{ position: 'absolute', left: '0px', color: '#8a96a8' }}>&#8226;</div>}

          {isNested ? (
            <details style={{ paddingTop: '68px', marginTop: '55px', color: '#2b313b', fontFamily: 'VTB Group UI', fontSize: '14px', fontStyle: 'normal', fontWeight: '600', lineHeight: '20px' }} open={true}>
            ...
            <NavLink
              to={item.href}
              style={{
                backgroundColor: isActive ? '#F6F9FB' : '#F6F9FB',
                borderRadius: 5,
                paddingLeft: '45px',
                color: '#2b313b',
